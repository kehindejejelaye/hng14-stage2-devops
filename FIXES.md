...
| `Dockerfiles` | N/A | Build failure `chown: unknown user/group appuser:appuser` | Fixed group name to `appgroup` in `chown` and `COPY --chown` commands across all Dockerfiles |
